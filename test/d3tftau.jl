function d3tftau(t,fft_u,fft_v,A1,A2,matr,conjmatr,sigma,llambda,Ntaumm)
    ...
    du=ifft(exp(1im*t*A1)*(1im*A1)*fft_u)*matr
    dv=ifft(exp(1im*t*A1)*(1im*A1)*fft_v)*matr
    d2u=ifft(exp(1im*t*A1)*(-A1.^2)*fft_u)*matr
    d2v=ifft(exp(1im*t*A1)*(-A1.^2)*fft_v)*matr
    d3u=ifft(exp(1im*t*A1)*(-1im*A1.^3)*fft_u)*matr
    d3v=ifft(exp(1im*t*A1)*(-1im*A1.^3)*fft_v)*matr
    ...
    champu3=-1im*llambda*A2*exp(-1im*t*A1)*(-A1.^2)*fft(conjmatr*fz1)
    champv3=-1im*llambda*A2*exp(-1im*t*A1)*(-A1.^2)*fft(conjmatr*conj(fz1))
       
    fz1=abs(z).^2*z
    champu4=-1im*llambda*A2*exp(-1im*t*A1)*(1im*A1.^3)*fft(conjmatr*fz1)
    champv4=-1im*llambda*A2*exp(-1im*t*A1)*(1im*A1.^3)*fft(conjmatr*conj(fz1))
